...
/** Detect the filter panel bounding box using brightness thresholding + projection. */
function detectFilterRegion(gray: Float32Array, w: number, h: number): Rect {
  ...
  // Scan inward to find the extents that have >30% bright pixels
...
/** Remove slow lighting gradient by subtracting a heavily smoothed version. */
...
/** Prominence-based peak detection — robust against noisy baselines. */
...
/** Score how periodic the peak spacing is. Higher = more pleat-like. */
...
function analyzePleats(
  ...
  // sensitivity 0→100 maps prominence factor 0.55→0.05 (lower = more peaks detected)
...
function drawOverlay(
  ...
  [[region.x, region.y, 1, 1], [region.x + region.w, region.y, -1, 1],
   [region.x, region.y + region.h, 1, -1], [region.x + region.w, region.y + region.h, -1, -1]]
    .forEach(([cx, cy, dx, dy]) => {
      ctx.beginPath();
  ...
  // Pleat indicator lines
  if (positions.length > 0) {
    ctx.strokeStyle = 'rgba(255, 82, 82, 0.55)';
    ctx.lineWidth = Math.max(1, cw / 700);
...
export default function FilterPleatCounterPage() {
  ...
  const areaMm2 = (pleatCount > 0 && !isNaN(widthMm) && !isNaN(phMm))
  ...
  return (
    <main>
      ...
      <div className="container">
        ...
        <div className="page-header">
          ...
          <h1 className="section-title">
            <span className="gradient-text">Machine-Vision Media Area</span>
          </h1>
          <p className="section-subtitle" style={{ marginBottom: 0 }}>
            Photograph a filter panel — pleats are auto-counted and useable media area calculated instantly
          </p>
        </div>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto 4rem' }}>
          ...
          {!imageSrc ? (
            ...
            >
              ...
              <p style={{ fontWeight: 700, fontSize: '1.1rem', marginBottom: '0.4rem' }}>
                Photograph your filter panel
              </p>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.75rem', maxWidth: 380, margin: '0 auto 1.75rem' }}>
                Point the camera at the filter — the panel outline and pleat count will be detected automatically
              </p>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {/* Legend */}
              <div style={{ display: 'flex', gap: '1.25rem', marginTop: '0.6rem', fontSize: '0.78rem', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
          ...
          {/* ── Sensitivity (only visible after capture) ── */}
          {imageSrc && (
            <div style={{
          ...
          <div className="calculator-card" style={{ marginTop: '1.25rem' }}>
            ...
            {/* Measurement inputs — 2-col on desktop, 1-col on mobile */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '0 1.25rem' }}>
              ...
              <div className="calc-field">
                <label className="calc-label">
                  Length <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(mm) &mdash; across pleats</span>
                </label>
                <input type="number" inputMode="decimal" className="calc-input"
      ...
      {/* Spinner keyframe injected inline — avoids modifying globals */}
